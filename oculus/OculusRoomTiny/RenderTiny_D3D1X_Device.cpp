...
Texture* RenderDevice::CreateTexture(int format, int width, int height, const void* data, int mipcount)
{
	...
	{
		dsDesc.Usage     = D3D1x_(USAGE_DYNAMIC);
		dsDesc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
	}
	else
	{
...
void RenderDevice::UpdateTexture( OVR::RenderTiny::Texture* texture, const void* data )
{
	Texture* d3dTex = (Texture*)texture;
	D3D10_MAPPED_TEXTURE2D mappedTexture;
	if ( SUCCEEDED( d3dTex->Tex->Map( 0, D3D10_MAP_WRITE_DISCARD, 0, &mappedTexture ) ) )
	{
		if ( mappedTexture.RowPitch == d3dTex->Width * 4 )
		{
			memcpy( mappedTexture.pData, data, d3dTex->Width * d3dTex->Height * 4 );
		}
		else
		{
			char* buffer = (char*)mappedTexture.pData;
			for( int y = 0 ; y < d3dTex->Height ; y++ )
			{
				for( int x = 0 ; x < d3dTex->Width ; x++ )
				{
					((unsigned int*)buffer)[x] = ((unsigned int*)data)[y * d3dTex->Width + x];
				}
				buffer += mappedTexture.RowPitch;
			}
		}
		d3dTex->Tex->Unmap( 0 );
	}
...
void RenderDevice::FillRect(float left, float top, float right, float bottom, Color c)
{
	...
	OVR::RenderTiny::RenderDevice::FillRect(left, top, right, bottom, c);
	Context->OMSetBlendState(NULL, NULL, 0xffffffff);
}

void RenderDevice::FillTexture(float left, float top, float right, float bottom, RenderTiny::Texture* texture)
{
	Context->OMSetBlendState(BlendState, NULL, 0xffffffff);
	OVR::RenderTiny::RenderDevice::FillTexture(left, top, right, bottom, texture);
	Context->OMSetBlendState(NULL, NULL, 0xffffffff);
}
